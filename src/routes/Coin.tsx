...
interface RouterParams extends Record<string, string> {
  coinId: string
}
interface InfoData {
  id: string;
...
const Coin = () => {
  ...
  const { isLoading: tickersLoading, data: ticker } = useQuery<TickerData>(["ticker", coinId], () => fetchCoinPriceInfo(coinId!), {
  ...
  const loading = useMemo(() => infoLoading || tickersLoading, [infoLoading, tickersLoading])
  return (
    <Container>
      <Helmet>
        <title>
